...
import { getLikeAndFollowState, unLikeComic,likeComic ,FollowComic,unFollowComic} from "@/service/backend/ComicControllers";



const ComicInfo = ({comic,first_chapter,noticeShow}: any) =>{
    ...
    const evaluteStarComic = async(rate_star: any)=>{
        if(!sessionStorage.getItem("access_token")){
            ...
            noticeShow('error','Bạn đã đánh giá rồi!');
        }else{
            setIsEvalute(true)
    ...
    return (
        ...
        <div className={classes.comicdetail__header}>
            <div className="main-image w-full h-[32rem] relative overflow-hidden">
                <img className="w-full h-full object-cover object-left-top filter blur-lg webkit-filter blur-md " src={comic.thumb} alt="Cover"/>
            </div>
            <Container>
                    ...
                    <div className={classes.detailinfo__right}>
                        ...
                        {comic.authors ?
                            <div className={classes.detailinfo__author}>
                                <span>Author: </span>
                                {comic.authors && comic.authors.map((ele: any) => (
                        ...
                        {comic.genres ?
                            <div className={classes.detailinfo__genre}>
                                {comic.genres && comic.genres.map((ele: any) => (
                                    <Link key={ele} href={'/search?filter_genre=' + ele + '&advance=true'}>
                                        {ele}
                                    </Link>
                                ))}
                            </div>
                        ...
                        {
                            comic.brief_desc ?
                                <div className="detailinfo__brief max-h-36 scrollbar-thin scrollbar-thumb-yellow-200 scrollbar-track-gray-200 overflow-y-scroll" dangerouslySetInnerHTML={{ __html: comic.brief_desc }}>
                                </div>
                                : <Skeleton variant="rectangular" height={20} animation="wave" />
                        }
                        <div className={classes.detailinfo__evaluate}>
                            <EnvalueStar comic={comic} evaluteStarComic={evaluteStarComic} percent_rating={percent_rating} ></EnvalueStar>
                            <div>
                                <span>Lượt xem: </span>
                        ...
                        <div className="flex gap-2">
                            <Link href={'/comic/' + comic.slug + '/'+''} className="py-2 px-4 bg-yellow-400 text-white rounded-md flex items-center">
                                <span>Đọc ngay</span>
                            </Link>
                            <button className={`py-2 px-4 rounded-md flex items-center ${isLike ? "bg-red-500 text-white" : "bg-white text-red-500 border border-red-500"}`} onClick={clickLikeComic}>
                                <FontAwesomeIcon icon={faHeart} />
                                <span>{isLike ? ' Đã yêu thích' : ' Thêm vào danh sách yêu thích'}</span>
                            </button>
                            <button id="button-follow" className={`py-2 px-4 rounded-md flex items-center ${isFollow ? "bg-blue-500 text-white" : "bg-white text-blue-500 border border-blue-500"}`} onClick={clickFollowComic}>
